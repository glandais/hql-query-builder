...
import net.ihe.gazelle.hql.generator.util.StringUtil;
...
public class MetaAttributeGenerationVisitor extends SimpleTypeVisitor6<AnnotationMetaAttribute, Element> {
	...
	/**
	 * FQCN of the Hibernate specific @Target annotation. We do not use the
	 * class directly to avoid depending on Hibernate Core.
	 */
	private static final String ORG_HIBERNATE_ANNOTATIONS_TARGET = "org.hibernate.annotations.Target";

	/**
	 * FQCN of the Hibernate specific @Type annotation. We do not use the class
	 * directly to avoid depending on Hibernate Core.
	 */
	...
	public AnnotationMetaAttribute visitPrimitive(PrimitiveType t, Element element) {
		return new AnnotationMetaSingleAttribute(entity, element, TypeUtils.toTypeString(t), AttributeType.PRIMITIVE);
	}
	...
	public AnnotationMetaAttribute visitArray(ArrayType t, Element element) {
		...
		// The code below is an attempt to be closer to the spec and only allow
		...
		// }
		// return attribute;
		return new AnnotationMetaSingleAttribute(entity, element, TypeUtils.toTypeString(t), AttributeType.ARRAY);
	}
	...
	public AnnotationMetaAttribute visitTypeVariable(TypeVariable t, Element element) {
		...
		return new AnnotationMetaSingleAttribute(entity, element, erasedType.toString(), AttributeType.VARIABLE);
	}
	...
	public AnnotationMetaAttribute visitDeclared(DeclaredType declaredType, Element element) {
		...
		// WARNING: .toString() is necessary here since Name equals does not
		// compare to String
		...
		if (collection != null) {
			...
			return new AnnotationMetaSingleAttribute(entity, element, type, AttributeType.BASIC);
		}
		return metaAttribute;
	}

	private AnnotationMetaAttribute createMetaCollectionAttribute(DeclaredType declaredType, Element element,
			String fqNameOfReturnType, String collection, String targetEntity) {
		if (TypeUtils.containsAnnotation(element, Constants.ELEMENT_COLLECTION)) {
			...
			TypeMirror collectionElementType = TypeUtils.getCollectionElementType(declaredType, fqNameOfReturnType,
					explicitTargetEntity, context);
			final TypeElement collectionElement = (TypeElement) context.getTypeUtils().asElement(collectionElementType);
			AccessTypeInformation accessTypeInfo = context.getAccessTypeInfo(collectionElement.getQualifiedName()
					.toString());
			if (accessTypeInfo == null) {
				AccessType explicitAccessType = TypeUtils.determineAnnotationSpecifiedAccessType(collectionElement);
				accessTypeInfo = new AccessTypeInformation(collectionElement.getQualifiedName().toString(),
						explicitAccessType, entity.getEntityAccessTypeInfo().getAccessType());
		...
		if (collection.equals("javax.persistence.metamodel.MapAttribute")) {
			...
			return new AnnotationMetaCollection(entity, element, collection, getElementType(declaredType, targetEntity), AttributeType.COLLECTION);
		}
	}
	...
	private AnnotationMetaAttribute createAnnotationMetaAttributeForMap(DeclaredType declaredType, Element element,
			String collection, String targetEntity) {
		String keyType;
		if (TypeUtils.containsAnnotation(element, Constants.MAP_KEY_CLASS)) {
			TypeMirror typeMirror = (TypeMirror) TypeUtils.getAnnotationValue(
					TypeUtils.getAnnotationMirror(element, Constants.MAP_KEY_CLASS),
					TypeUtils.DEFAULT_ANNOTATION_PARAMETER_NAME);
			keyType = typeMirror.toString();
		} else {
			keyType = TypeUtils.getKeyType(declaredType, context);
		}
		return new AnnotationMetaMap(entity, element, collection, keyType, getElementType(declaredType, targetEntity), AttributeType.MAP);
	}
	 ...
	 * 
	 * @return target entity class name as string or {@code null} if no
	 *         targetEntity is here or if equals to void
	 */
...
/**
 * Checks whether the visited type is a basic attribute according to the JPA 2
 * spec ( section 2.8 Mapping Defaults for Non-Relationship Fields or
 * Properties)
 */
